...
int main(int argc, char** argv) {
    int init_agent_count = 300; // 300 seems to be lower limit for simulations w/o population adjustments
    int window_w = 1080;
    int window_h = 720;
    ...
    auto simulation = swarmulator::Simulation(window_w, window_h, world_size, subdivisions, 0.1, 1000000/*100000000*/, log_path, 4, 20); // 100 million (1e8) updates at 0.1 dt each is ten million (1e7) simulation time
    ...
    auto watcher = swarmulator::PredPreyWatcher({0, 0, 0}, {0, 0, 0}, sixth * 3, 2, 5); //  we always need at least 1 predator to keep prey population in check
