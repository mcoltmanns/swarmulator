...
#include "agent/PredatorAgent.h"
#include "agent/PreyAgent.h"
#include "util/util.h"
...
int main(int argc, char** argv) {
    ...
    // initialize all the agents (0.25/0.75 predator/prey)
    for (int i = 0; i < init_agent_count / 4; i++) {
        ...
        auto a = std::make_shared<swarmulator::agent::PredatorAgent>(xyz(p), xyz(r));
        a->mutate();
        simulation.add_agent(a);
    }
    for (int i = 0; i < init_agent_count * 3 / 4; i++) {
        const auto p = Vector4{(randfloat() - 0.5f) * world_size.x, (randfloat() - 0.5f) * world_size.y, (randfloat() - 0.5f) * world_size.z, 0};
        const auto r = Vector4{randfloat() - 0.5f, randfloat() - 0.5f, randfloat() - 0.5f, 0};
        auto a = std::make_shared<swarmulator::agent::PreyAgent>(xyz(p), xyz(r));
        a->mutate();
        simulation.add_agent(a);
    }
