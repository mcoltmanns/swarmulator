...
min_step = int(sys.argv[5])
max_step = int(sys.argv[6])
train_epochs = 50
...
with tqdm(desc=f"{artifact_path}", total=len(steps) * len(chunk_starts)) as bar:
    for s in steps:
        ...
        for chunk_start in chunk_starts:
            ...
            print('novelty')
            score, losses, training_losses = novelty(arts, t, lookback, s, chunk_start, lookback, train_epochs) # try to predict as far forward as we do backwards
            if score is not None and training_losses is not None:
                ns.append(score)
                nls.append(losses)
