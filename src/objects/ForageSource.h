...
namespace swarmulator {
    ...
    private:
        float interaction_radius_ = 100; // radius should be large enough to cover whole environment, or close to it
            ...
            for (const auto& object : neighborhood) {
                ...
                if (const auto forager = dynamic_cast<NeuralAgent*>(object)) {
                    ...
                    const float bonus = 1.f /
                        (1.f / max_gain + dist * (context.get_total_num_objects() / 2000.f)); // 2500 is a soft object cap
