...
namespace swarmulator {
    ...
    void Agent::directional_update(const std::vector<Agent *> &neighborhood, const float dt) {
        for (const auto neighbor : neighborhood) {
            auto distance = Vector3Distance(position_, neighbor->position_);
            if (neighbor == this || distance > sense_range_) {
                continue;
            }
            const auto [x, y, z] = direction_ - Vector3Normalize(neighbor->position_ - position_); // diffs tells us which cardinal segment the neighbor is in
